using DataFrames, LaTeXStrings, JLD2, HEPAnalysisUtils, Measurements
...
write("tab.tex", latexify(latexify.(df), latex=false, env=:table, head=["Source" "Factor" "Uncertainty (\\%)"], adjustment=[:c, :c, :r], booktabs=true))
write("tab.md", latexify(latexify.(df), latex=false, env=:mdtable, head=["Source" "Factor" "Uncertainty (%)"], adjustment=[:c, :c, :r]) |> string)
